...
export class SalidasService {
  ...
  async findAll(query: {
    ...
    if (query.search) {
      qb.andWhere('(LOWER(r.nombre) LIKE LOWER(:search) OR LOWER(ft.nombre) LIKE LOWER(:search))', {
        search: `%${query.search}%`,
      });
    }
  ...
  async findOne(id: number) {
    const salida = await this.salidasRepo.findOne({
      where: { id },
      relations: ['recurso', 'recurso.categoria', 'frenteTrabajo', 'quienEntrega', 'quienRecibe'],
    });
  ...
  async create(dto: CreateSalidaDto, userId: string) {
    ...
    const salida = this.salidasRepo.create({
      ...dto,
      created_by: userId,
    });
    ...
    await this.dataSource.query(
      ...
      ['SALIDA', saved.id, dto.recurso_id, dto.cantidad, dto.fecha, `${dto.descripcion_trabajo || ''}`, userId],
    );
